...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Aggregate per-target probe baseline evaluator outputs into "
            "cross-target summary tables for a fixed train domain."
        )
    )
    parser.add_argument(
        "--teacher_metrics_csv",
        ...
        required=True,
        help=(
            "Path to teacher_metrics_<tag>.csv from evaluate_probe_level_baselines.py. "
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Path to baseline_metrics_<tag>.csv from evaluate_probe_level_baselines.py. "
            "Provide one per target domain."
        ),
    )
    parser.add_argument("--outdir", type=str, required=True)
    parser.add_argument("--tag", type=str, default="cross_target_probe")
    ...
    # Optional consistency checks on baseline data
    ...
    baseline_df.to_csv(outdir / f"all_baseline_metrics_{args.tag}.csv", index=False)

    print("\n=== Loaded baseline methods ===")
    ...
    # --------------------------------------------------------
    # All per-target baselines already computed
    # KEEP ALL METHODS, including modern baselines
    # --------------------------------------------------------
    ...
    baseline_methods_df = baseline_methods_df.sort_values(
    ...
    # Keep this file name for backward compatibility, but now it contains all methods
    baseline_methods_df.to_csv(
        outdir / f"ensemble_and_confidence_methods_{args.tag}.csv",
        index=False,
    )
    ...
    final_baseline_df = final_baseline_df.sort_values(
        ["target_domain", "method", "selected_teacher"],
        ascending=[True, True, True],
    ).reset_index(drop=True)
...
# # ---------------------------------
# # Teacher CSVs: classical only
# # ---------------------------------
# $teacherCsvs = @(
#     "terra_probe_results\probe_baselines\$trainDomain\location_38\teacher_metrics_terra_incognita_${trainDomain}_location_38.csv",
#     "terra_probe_results\probe_baselines\$trainDomain\location_43\teacher_metrics_terra_incognita_${trainDomain}_location_43.csv",
#     "terra_probe_results\probe_baselines\$trainDomain\location_46\teacher_metrics_terra_incognita_${trainDomain}_location_46.csv",
#     "terra_probe_results\probe_baselines\$trainDomain\location_100\teacher_metrics_terra_incognita_${trainDomain}_location_100.csv"
# )

# # ---------------------------------
# # Baseline CSVs: classical + modern
# # ---------------------------------
# $baselineCsvs = @(
#     # classical
#     "terra_probe_results\probe_baselines\$trainDomain\location_38\baseline_metrics_terra_incognita_${trainDomain}_location_38.csv",
#     "terra_probe_results\probe_baselines\$trainDomain\location_43\baseline_metrics_terra_incognita_${trainDomain}_location_43.csv",
#     "terra_probe_results\probe_baselines\$trainDomain\location_46\baseline_metrics_terra_incognita_${trainDomain}_location_46.csv",
#     "terra_probe_results\probe_baselines\$trainDomain\location_100\baseline_metrics_terra_incognita_${trainDomain}_location_100.csv",

#     # modern
#     "terra_probe_results\probe_baselines_modern\$trainDomain\location_38\baseline_metrics_terra_incognita_${trainDomain}_location_38.csv",
#     "terra_probe_results\probe_baselines_modern\$trainDomain\location_43\baseline_metrics_terra_incognita_${trainDomain}_location_43.csv",
#     "terra_probe_results\probe_baselines_modern\$trainDomain\location_46\baseline_metrics_terra_incognita_${trainDomain}_location_46.csv",
#     "terra_probe_results\probe_baselines_modern\$trainDomain\location_100\baseline_metrics_terra_incognita_${trainDomain}_location_100.csv"
# )
...
# python scripts\agreggate_probe_baseline_results.py `
#     @argsList `
#     --outdir "terra_probe_results\probe_baselines_combined\$trainDomain\cross_target_summary" `
#     --tag "terra_incognita_${trainDomain}_combined"